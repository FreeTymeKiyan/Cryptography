...
class AES:
    ...
    def encrypt(self, inString):
        """Encrypt plaintext passed as a string"""
        try:
            assert self.keyDefined
        except AssertionError:
            print("Key not defined")
            sys.exit("ValueError")
        inList = self.padData(inString)
        outList = []
        if self.cipherMode == "MODE_CBC":
    ...
    def decrypt(self, inList):
        """Decrypt ciphertext in byte list form with given key"""
        try:
            assert self.keyDefined
        except AssertionError:
            print("Key not defined")
